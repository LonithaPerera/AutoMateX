...
<div class="max-w-lg mx-auto px-4 pt-5 pb-24">
    ...
    <div class="flex items-start justify-between mb-5 fade-in fade-in-1">
        <div>
            ...
            <h1 class="heading text-3xl font-bold text-white">{{ __('app.garage_bookings_title') }}</h1>
            <p class="text-xs mt-1" style="color:#64748b;">{{ __('app.garage_bookings_desc') }}</p>
        </div>
    ...
    @endphp
    <div class="grid grid-cols-4 gap-2 mb-4 fade-in fade-in-2">
        <div class="rounded-2xl p-3 text-center border" style="background:rgba(251,191,36,0.05);border-color:rgba(251,191,36,0.15);">
            <p class="heading text-xl font-bold" style="color:#fbbf24;">{{ $pending }}</p>
        ...
        <div class="rounded-2xl p-3 text-center border" style="background:rgba(0,245,255,0.05);border-color:rgba(0,245,255,0.15);">
            <p class="heading text-xl font-bold text-cyan">{{ $confirmed }}</p>
        ...
        <div class="rounded-2xl p-3 text-center border" style="background:rgba(74,222,128,0.05);border-color:rgba(74,222,128,0.15);">
            <p class="heading text-xl font-bold" style="color:#4ade80;">{{ $completed }}</p>
        ...
        <div class="rounded-2xl p-3 text-center border" style="background:rgba(248,113,113,0.05);border-color:rgba(248,113,113,0.15);">
            <p class="heading text-xl font-bold" style="color:#f87171;">{{ $cancelled }}</p>
            ...
            <div class="p-4 rounded-2xl border" style="background:rgba(0,10,30,0.5);border-color:rgba(0,102,255,0.15);">
                ...
                <div class="flex items-center gap-4 mt-3 pt-3" style="border-top:1px solid rgba(255,255,255,0.05);">
                    <div class="flex items-center gap-1.5">
                        ...
                        <span class="text-xs" style="color:#64748b;">{{ __('app.pending') }}</span>
                    </div>
                    <div class="flex items-center gap-1.5">
                        <div class="w-2.5 h-2.5 rounded-full" style="background:#00f5ff;"></div>
                        <span class="text-xs" style="color:#64748b;">{{ __('app.confirmed') }}</span>
                    </div>
                    <div class="flex items-center gap-1.5">
                        <div class="w-2.5 h-2.5 rounded-full" style="background:#4ade80;"></div>
                        <span class="text-xs" style="color:#64748b;">{{ __('app.completed') }}</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    {{-- Filter tabs --}}
    <div class="flex gap-2 mb-3 fade-in fade-in-3 overflow-x-auto pb-1">
        @foreach([
    ...
    {{-- Search bar --}}
    <div class="relative mb-4 fade-in fade-in-3">
    ...
    @else
        ...
        <div class="booking-card glass-bright rounded-2xl p-4 mb-4 border fade-in"
            ...
            <form method="POST" action="{{ route('bookings.update', $booking) }}" class="mb-3">
                @csrf @method('PATCH')
            ...
            {{-- Garage Note --}}
            <div style="border-top:1px solid rgba(0,245,255,0.08);padding-top:12px;margin-bottom:12px;">
...
function setHeaders(showAll) {
    ['header-overdue','header-upcoming-inner','header-history','empty-upcoming'].forEach(function(id) {
...
function filterBookings(filter) {
    ...
    var query = document.getElementById('bookingSearch').value.toLowerCase();
    ...
    cards.forEach(function(card) {
        var sm = filter === 'all' || card.dataset.status === filter;
        var qm = query === '' || card.dataset.customer.includes(query) || card.dataset.plate.includes(query);
...
function searchBookings(value) {
    var filter = window._activeFilter || 'all';
