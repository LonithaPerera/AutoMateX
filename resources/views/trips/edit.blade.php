...
<div class="max-w-lg mx-auto px-4 pt-5 pb-8">
    ...
    <div class="mb-5 fade-in fade-in-1">
        <nav class="flex items-center gap-1.5 text-xs mb-3" style="color:#64748b;">
            ...
            <span>›</span>
            <span style="color:#94a3b8;">{{ __('app.edit_trip_title') }}</span>
        </nav>
        <p class="section-label mb-1">{{ __('app.trip_log_label') }}</p>
        <h1 class="heading text-3xl font-bold text-white">{{ __('app.edit_trip_title') }}</h1>
    ...
    <div class="glass-bright rounded-2xl p-5 fade-in fade-in-2 border animate-glow">
        <form method="POST" action="{{ route('trips.update', [$vehicle, $tripLog]) }}">
            @csrf @method('PATCH')
            ...
            <div class="mb-4">
                ...
                <input type="date" name="trip_date"
                       value="{{ old('trip_date', $tripLog->trip_date->format('Y-m-d')) }}" required
            ...
            <div class="grid grid-cols-2 gap-3 mb-4">
                <div>
                    ...
                    <input type="number" name="start_km"
                           value="{{ old('start_km', $tripLog->start_km) }}"
                           required min="0"
                ...
                <div>
                    ...
                    <input type="number" name="end_km"
                           value="{{ old('end_km', $tripLog->end_km) }}"
                           required min="0"
                           class="w-full px-4 py-3 rounded-xl text-sm text-white placeholder-slate-600 outline-none"
                ...
                <div class="grid grid-cols-3 gap-2">
                    ...
                    <label class="relative cursor-pointer">
                        <input type="radio" name="purpose" value="{{ $p }}"
                               {{ old('purpose', $tripLog->purpose) === $p ? 'checked' : '' }}
            ...
            <div class="mb-6">
                ...
                <textarea name="notes" rows="2"
                          ...
                          style="background:rgba(255,255,255,0.04);border:1px solid rgba(0,245,255,0.15);">{{ old('notes', $tripLog->notes) }}</textarea>
            </div>
